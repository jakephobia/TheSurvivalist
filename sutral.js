...
function updateEpisodeTable(epId, episodeDataIndex, allEpisodeData) {
  ...
  // Power rankings per episodio: ordina e classifica per EpPts (baseRaw), non TotPts
  const sortedByEpPts = playersWithTotals.slice().sort((a, b) => b.baseRaw - a.baseRaw);
  sortedByEpPts.forEach((p, i) => { p.epRank = i + 1; });
  const n = sortedByEpPts.length;
  const useTwoCols = n >= 8;
  const leftCount = Math.floor(n / 2);
  const makeTable = (arr) => `
    <table class="sutral-power-table">
      <thead><tr><th>Pos</th><th>Name</th><th>Ep Pts</th></tr></thead>
      <tbody>${arr.map(p => {
        const color = getPlayerColor(p.name);
        return `<tr><td>${p.epRank}</td><td><span class="player-color-dot" style="background:${color}"></span>${p.name}</td><td>${p.baseRaw.toFixed(2)}</td></tr>`;
      }).join('')}</tbody>
    </table>`;
  let tableHtml;
  if (useTwoCols) {
    const left = sortedByEpPts.slice(0, leftCount);
    const right = sortedByEpPts.slice(leftCount);
    tableHtml = `<div class="sutral-power-two-cols"><div>${makeTable(left)}</div><div>${makeTable(right)}</div></div>`;
  } else {
    tableHtml = makeTable(sortedByEpPts);
  }
  if (rankingDiv) {
